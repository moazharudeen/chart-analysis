        ...
        if 'csrfmiddlewaretoken' in post_data:
            ...
            if chart_type in chart_dict:
                if chart_type == 'column2d':
                    ...
                    script_dict_chart = script_dict.get(chart_type)
                    print("script_dict_chart=============", script_dict_chart)
                    ...
                    javascript_script = script_dict_chart.format(data, title[0], sub_title[0], x_axis_name[0],y_axis_name[0], number_suffix[0], chartid, res )
                    ...
                    filter_script_data = script_dict_chart.format('replaceable_data', title[0], sub_title[0], x_axis_name[0],y_axis_name[0], number_suffix[0], chartid, res )
                    print("====================script======================")
                    print(script)
                    filter_chart_data = filter_chart_data.replace('flower_close', '}').replace('flower_open', '{')
                    javascript_script = javascript_script.replace('flower_close', '}').replace('flower_open', '{')
                    filter_script_data = filter_script_data.replace('flower_close', '}').replace('flower_open', '{')
                    script = script.replace('flower_close', '}').replace('flower_open', '{')
                    print("====================new script======================")
                    print(script)
                    request.session['script'] = script
                    request.session['javascript_script'] = javascript_script
                    request.session['filter_script_data'] = filter_script_data
                    request.session['chart_type'] = chart_type
                    request.session['query'] = query
                    request.session['filter_chart_data'] = filter_chart_data
                    request.session['chart_unique_id'] = str(chartid)
    ...
    if request.method == 'POST':
        ...
        if 'csrfmiddlewaretoken' in post_data:
            chartData.objects.create(
                ...
                filter_script_data = request.session.get('filter_script_data'),
                javascript_script = request.session.get('javascript_script'),
                chart_unique_id = request.session.get('chart_unique_id')
...
def create_dashboard2(request):
    ...
    charts_name_query = chartData.objects.values_list('chart_name', 'chart_data')
    print(dict(charts_name_query))
    charts_name = dict(charts_name_query)
    ...
    if request.method == 'POST':
        ...
        final_script = post_data.get('body_script')[0]
        request.session['final_script'] = final_script
        request.session['dashboard_name'] = dashboard_name
        if 'csrfmiddlewaretoken' in post_data:
            ...
            return HttpResponse("Success")
             #post_data.get('dashboard_name')[0]
            # filters = post_data.get('filters')
            # filter_dict = {}
            # for i in filters:
            #     filter_dict[i] = df[i].unique()
            ...
            # }
            # return render(request, 'create_dashboard2.html', context)
...
def dashboard_view(request):
    if request.method == 'GET':
        ...
        if 'filter_status' in get_data:
            ...
            javascript_scripts = chartData.objects.values_list('filter_script_data', flat=True)
        else:
            # print('uuid', uuid)
            quried_data = dashboard.objects.filter(
                id=uuid
            ).values_list('dashboard_data', 'dashboard_name')
            javascript_scripts = chartData.objects.values_list('javascript_script', flat=True)
            # print(quried_data)
            context = {
                'data':quried_data[0][0],
                'javascript_scripts':javascript_scripts,
                'id':uuid,
